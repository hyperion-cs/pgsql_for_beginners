...
1. [Сообщество](#1-сообщество-family_man_woman_girl_boy)
2. [Вопросы](#2-вопросы-interrobang)<br>
    2.1. [Не могу подсоединиться к PostgreSQL. Что делать?](#21-не-могу-подсоединиться-к-postgresql-что-делать)<br>
    2.2. [Получаю ошибку что имя таблицы/колонки/etc неверное, хотя это не так. В чем дело?](#22-получаю-ошибку-что-имя-таблицыколонкиetc-неверное-хотя-это-не-так-в-чем-дело)<br>
    2.3. [Как публиковать SQL запросы, выводы команд вспомогательных утилит и прочую текстовую информацию?](#23-как-публиковать-sql-запросы-выводы-команд-вспомогательных-утилит-и-прочую-текстовую-информацию)<br>
    2.4. [У меня медленно работает SQL запрос и я хочу попросить помощи у сообщества. Какую информацию мне необходимо предоставить?](#24-у-меня-медленно-работает-sql-запрос-и-я-хочу-попросить-помощи-у-сообщества-какую-информацию-мне-необходимо-предоставить)<br>
    2.5. [У меня не получается написать SQL запрос и/или я получаю ошибки. Какую информацию мне необходимо предоставить для получения помощи?](#25-у-меня-не-получается-написать-sql-запрос-иили-я-получаю-ошибки-какую-информацию-мне-необходимо-предоставить-для-получения-помощи)<br>
    2.6. [В каком регистре писать команды/функции/etc в PostgreSQL?](#26-в-каком-регистре-писать-командыфункцииetc-в-postgresql)<br>
3. [Книги](#3-книги-blue_book)
4. [Полезные ссылки](#4-полезные-ссылки-link)
5.  [Переводы / Translations](translations)<br>
    5.1. [English](translations/ENG.md)


## 1. Сообщество :family_man_woman_girl_boy:
...
### 2.3 Как публиковать SQL запросы, выводы команд вспомогательных утилит и прочую текстовую информацию?

Прежде всего, **не используйте** скриншоты для того, чтобы показать SQL запросы (и/или их результаты)
или выводы команд вспомогательных утилит (таких, как [psql](https://postgrespro.ru/docs/postgresql/15/app-psql) и др.).
Заметно эффективнее будет публикация оных, напр., на [Github Gist](https://gist.github.com/), [pastebin.com](https://pastebin.com/)
или, на худой конец (если вы задаете вопрос в tg-чате и кол-во текста невелико), опубликовать текстом в режиме [форматирования Monospace](https://telegra.ph/markdown-07-07).
...
:exclamation: Внимательно отнеситесь к тому, как [публиковать информацию](#23-как-публиковать-sql-запросы-выводы-команд-вспомогательных-утилит-и-прочую-текстовую-информацию),
...
### 2.5. У меня не получается написать SQL запрос и/или я получаю ошибки. Какую информацию мне необходимо предоставить для получения помощи?
...
Единственный технический нюанс, который следует учитывать, описан [выше](#22-получаю-ошибку-что-имя-таблицыколонкиetc-неверное-хотя-это-не-так-в-чем-дело).
